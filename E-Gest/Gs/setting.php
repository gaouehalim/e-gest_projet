<?php
    ...
    $req = "SELECT * FROM site WHERE idUtilisateur = :id";   
    $site = $base_com->prepare($req);
    $site->bindParam(':id', $id, PDO::PARAM_INT);
    $site->execute();    
    $lessite = $site->fetchAll(PDO::FETCH_ASSOC); 
    foreach ($lessite as $lesite) {
        $nomSite = $lesite['nomSite'];
        $emailSite = $lesite['emailSite'];
        $contactSite = $lesite['contactSite'];
        $adresseSite = $lesite['adresseSite'];
        $cover = $lesite['cover'];
        $dateCreat = $lesite['dateCreat'];
        $idSite = $lesite['idSite'];
    }
