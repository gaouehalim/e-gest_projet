<?php
    ...
    $count = "SELECT COUNT(idAttribution) AS nombreMaterielle FROM attribution where idSite =?";
    $result = $base_com->prepare($count);
    $result->execute([$idSite]);
    $resulta = $result->fetch(PDO::FETCH_ASSOC);

    $recupMat = "SELECT att.*, rm.*, cm.*
    FROM attribution AS att
    INNER JOIN ressourcematerielle AS rm ON att.idMaterielle = rm.idMaterielle
            ...
            ?>
                    ...
                    <div class="flex items-center py-4 overflow-x-auto whitespace-nowrap">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                            ...
                            <span class="mx-2">Nos materielles</span>
                    ...
                    <section class="container px-4 mx-auto mt-10">
                        <div class="relative flex flex-col min-w-0 break-words border border-dashed bg-clip-border rounded-2xl border-stone-200 bg-white mb-5 draggable">
                            ...
                            <h3 class="flex flex-col items-start justify-center m-2 ml-0 font-medium text-xl/tight text-dark font-display">
                                <span class="mr-3 font-semibold text-dark">Nos Ressources matérielles<span class="px-3 py-1 text-5xl text-green-500 bg-green-100 rounded-full dark:text-green-500"><?php echo $resulta['nombreMaterielle'];?></span></span>   
                                
                            </h3>
                            <div class="relative flex flex-wrap items-center my-2">
                                <a href="demande.php" class="inline-block text-[.925rem] font-medium leading-normal text-center align-middle cursor-pointer rounded-2xl transition-colors duration-150 ease-in-out text-green-400 hover:text-white bg-white border-green-400 shadow-none border-2 py-2 px-5 hover:bg-green-400 active:bg-white focus:bg-white">Faire une demande</a>
                            </div>
                            </div>
                            <!-- end card header -->
                            <!-- card body  -->
