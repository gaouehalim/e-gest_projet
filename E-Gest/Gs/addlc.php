...
if(isset($_POST['envoyer'])) {
    ...
    if( !empty($typeLocal) && !empty($capaciteLocal) && !empty($etatLocal)){
        ...
        $stmt_annonce = $base_com->prepare("INSERT INTO ressourceimmobiliere (nomLocal, typeLocal, capaciteLocal, etatLocal, statutLocal, idSite, dateAdd, description, reservable,tarifLocal) VALUES (:nomLocal, :typeLocal, :capaciteLocal, :etatLocal, :statutLocal, :idSite, NOW(), :description, :reservable,:tarifLocal)");
        $stmt_annonce->execute(array(
            ':nomLocal' => $nomLocal,
            ':typeLocal' => $typeLocal,
            ':capaciteLocal' => $capaciteLocal,
            ':etatLocal' => $etatLocal,
            ':statutLocal' => 'disponible', 
            ':idSite' => $idSite ,
            ':description'=> $description,
            ':reservable' => isset($_POST['reservable']) ? 'oui' : 'non',
            ':tarifLocal' => $tarifLocal // Vérifie si la checkbox est cochée
        ));

    $nouvelId = $base_com->lastInsertId();

    
    $codeLocal = troisPremieresLettres($typeLocal).'000'.$nouvelId.$nomSite; 

    $req2 = "UPDATE ressourceimmobiliere SET codeLocal = ? WHERE idLocal = ?";
    $stmt2 = $base_com->prepare($req2);
    $stmt2->execute([$codeLocal, $nouvelId]);

    $message = "Ajout réussi du local de type $typeLocal <br> Voici le code unique du local <span class='text-red-500 font-bold'>$codeLocal</span>";
    ...
    if(isset($_FILES['images'])) {
        ...
        foreach($_FILES['images']['tmp_name'] as $key => $tmp_name) {
            ...
            $nom_image = $nouvelId.$nomSite.'_' . $_FILES['images']['name'][$key];
            ...
            if(move_uploaded_file($tmp_name, $chemin_image)) {
                ...
                $erreur="Erreur lors du téléchhargement des images.";
            }
        }
    }
    }else{
                            ...
                            <div class="mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                                ...
                                <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                                        <section class="mt-5 max-w-4xl p-6 mx-auto bg-white border-2 border-green-400 rounded-md shadow-md dark:bg-gray-800">
                                            ...
                                            <form  method="post" enctype="multipart/form-data" action="addlc.php">
